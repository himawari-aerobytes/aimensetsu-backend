...
from rag_sample_app.models import Document, Thread, ChatHistory, User
...
class ChatHistoryModelTest(TestCase):
    ...
    def test_message_creation(self):
        chat = ChatHistory.objects.create(thread_id=self.thread, message="Hello, this is a test message.", sender="User1")
    ...
    def test_timestamp_auto_now_add(self):
        chat = ChatHistory.objects.create(thread_id=self.thread, message="Another message", sender="User2")
    ...
    def test_foreign_key_relation_with_thread(self):
        chat = ChatHistory.objects.create(thread_id=self.thread, message="Thread relation test", sender="User1")
        self.assertEqual(chat.thread_id, self.thread)

class UserModelTest(TestCase):
    def test_create_user(self):
        user = get_user_model().objects.create_user(username="user1@example.com", password="password123")
        self.assertEqual(user.username, "user1@example.com")
        self.assertTrue(user.check_password("password123"))

    def test_email_is_unique(self):
        user1 = get_user_model().objects.create_user(username="unique@example.com", password="password123")
        with self.assertRaises(Exception):
            user2 = get_user_model().objects.create_user(username="unique@example.com", password="password456")